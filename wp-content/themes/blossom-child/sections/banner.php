...
<?php
    if ($hero_img = carbon_get_post_meta(get_the_ID(), 'crb_hero_img')){
        ?>
        <div class="hero-image">
            <img src="<?php  echo wp_get_attachment_image_url($hero_img, 'full'); ?>" alt="">
        </div>
        <?php
    }
?>
            ...
            <?php 
                 ...
                 if( $banner_title || $banner_subtitle || ( $button1 && $button1_url ) || ( $button2 && $button2_url ) ){
                     ...
                     if( $banner_title ) echo '<div class="hero-title"><h2 class="title">' . esc_html( $banner_title ) . '</h2>';
                     if( $banner_subtitle ) echo '<div class="description">' . wpautop( wp_kses_post( $banner_subtitle ) ) . '</div></div>';
                     if( $button1 || $button2 ) : ?>
                        <div class="btn-wrap">
                            <?php if( $button1 && $button1_url ) : ?>
                                <a href="<?php echo esc_url( $button1_url ); ?>" class="btn">
